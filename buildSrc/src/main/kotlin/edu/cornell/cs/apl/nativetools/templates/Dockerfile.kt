...
internal val linuxDockerfile = Platform.LINUX_64.let { platform ->
    ...
    ## Install dependencies
    COPY conanfile.* .
    RUN conan install . --install-folder=$cmakeSourceBuildDirectory --build=missing
...
internal val macosDockerfile = Platform.MACOS_64.let { platform ->
    ...
    ## Install dependencies
    COPY conanfile.* .
    RUN conan install . --install-folder=$cmakeSourceBuildDirectory --build=b2 --build=missing
